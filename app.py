    ...
    for chart in CHARTS:
        ...
        for idx in order:
            with cols[order.index(idx)%3]:
                ...
                minute=st.selectbox("Minute",minute_options,key=f"{key}_min")
...
def show_step2():
    ...
    for chart in CHARTS:
        ...
        with st.form(f"form_{chart}", clear_on_submit=True):
            ...
            minute=c4.selectbox("Minute",minute_options)
        ...
        # Show planet list with delete btns
...
# ===========================================================
# STEP3 — DISPLAY + XLSX MULTI-SHEET EXPORT
# ===========================================================
def show_step3():
    st.title("🔍 Synastry Result Table")

    # Sign → numeric mapping
    ...
    # Build sheet for each chart
    def build_sheet(reference):
        # Ensure cusp exists
        if any(h is None for h in st.session_state.houses[reference]):
            return None

        cusps=[to_long(*h) for h in st.session_state.houses[reference]]
    ...
    df_preview = build_sheet(st.session_state.mode)
    ...
    st.download_button("📥 Download Full Synastry Workbook (XLSX)",
        buffer.getvalue(),
